/**
 * Nodo n8n: "2. Rutear interacción de revisión"
 * Tipo: Code (JavaScript, "Run Once for Each Item")
 * Disparado por el Telegram Trigger "Recibir interacciones de revisión"
 * (updates: callback_query, message). Es el único nodo de todo el
 * workflow -- hace TODAS las llamadas (Supabase + Telegram Bot API)
 * directamente, sin depender de nodos nativos de Telegram, para no
 * arrastrar incertidumbre de nombres de parámetros entre versiones de
 * n8n (mismo criterio que el resto de la Fábrica de Reels).
 *
 ...
 *   varr:<render_id>          pedir variante automática (pide tipo primero)
 *   vt:<code>:<render_id>     tipo de variante elegido (pide comentario opcional)
 *   rlnk:<render_id>          regenerar link de revisión (utilitario, sin sesión)
 *   rrnd:<render_id>          volver a renderizar un video ya aprobado/revisado
 *   retr:<raw_video_id>       reintentar un render que FALLÓ (viene de Capa 4)
 *
...
// vencido") -- si falla, no es crítico: la decisión ya se guarda en
// Supabase igual, así que nunca dejamos que esto tumbe el resto del flujo.
...
const PERMISSION_MESSAGES = {
  render_not_found: '⚠️ No encontré este render en la base de datos.',
  raw_video_not_found: '⚠️ No encontré el video asociado.',
  not_a_reviewer: '🚫 No tenés permiso para revisar videos de este cliente.',
};
...
const createPublicationJob = async (render) => {
  ...
  // acá solo se puebla, no se crea una tabla `publish_jobs` nueva.
...
const regenerateLink = async (render, chatId, callbackQueryId) => {
  ...
  try {
    ...
    await safeAnswerCallback(callbackQueryId, '⚠️ No se pudo regenerar el link, probá de nuevo en un momento.');
  ...
  await tg.send(chatId, `🔗 Nuevo link de revisión (válido 7 días más): ${newUrl}`);
  return [{ json: { skip: false, render_id: render.id, decision: 'link_regenerated' } }];
};

// ---------- acciones que abren una sesión de comentario ----------
...
const startCommentSession = async (render, pendingAction, reviewerTelegramId, chatId, messageId, callbackQueryId) => {
  const promptText = pendingAction === 'error_flagged'
    ? '🚩 Contame QUÉ está mal (obligatorio) respondiendo a este mensaje:'
    : '✍️ Contame el motivo (o escribí "skip" para no dejar comentario) respondiendo a este mensaje:';
...
const startVariantTypeSelection = async (render, reviewerTelegramId, chatId, messageId, callbackQueryId) => {
  const replyMarkup = {
    inline_keyboard: [
      [{ text: 'Nuevo hook', callback_data: `vt:nh:${render.id}` }, { text: 'Subtítulos más agresivos', callback_data: `vt:sa:${render.id}` }],
      [{ text: 'Versión más corta', callback_data: `vt:vc:${render.id}` }, { text: 'Versión más limpia', callback_data: `vt:vl:${render.id}` }],
      [{ text: 'Cambio de template (futuro)', callback_data: `vt:ct:${render.id}` }, { text: 'Corrección de texto', callback_data: `vt:cx:${render.id}` }],
    ],
  };
  ...
  await safeAnswerCallback(callbackQueryId, 'Elegí el tipo de variante');
  await tg.editText(chatId, messageId, '¿Qué tipo de variante querés?', replyMarkup);
...
const chooseVariantType = async (renderId, code, reviewerTelegramId, chatId, messageId, callbackQueryId) => {
  ...
  if (!session) {
    await safeAnswerCallback(callbackQueryId, '⚠️ Esta sesión ya no está activa (¿expiró o ya se resolvió?).');
    return [{ json: { skip: true, reason: 'session_not_found' } }];
  }

  const sentPrompt = await tg.send(chatId, '✍️ Contame en una frase qué tenés en mente para esta variante (o escribí "skip" para no dejar comentario):');
...
// ---------- finalización de sesiones de comentario ----------
...
const finalizeWithComment = async (session, comment, chatId) => {
  ...
  const labelMap = {
    rejected: '❌ Rechazado',
    needs_changes: '✏️ Cambios solicitados',
...
const handleTextMessage = async (message) => {
  ...
  const session = sessions[0];
  if (!session) {
    // No hay ninguna revisión esperando comentario en este chat -- se ignora.
    return [{ json: { skip: true, reason: 'no_active_session' } }];
  }

  if (session.pending_action === 'error_flagged' && /^(skip|-)$/i.test(text)) {
    await tg.send(chatId, '🚩 "Marcar como error" necesita una descripción -- contame qué está mal, no puede quedar vacío.');
...
const handleCallbackQuery = async (cbq) => {
  ...
  if (action === 'retr') {
    ...
    if (!permission.ok) {
      await safeAnswerCallback(cbq.id, PERMISSION_MESSAGES[permission.reason] || '⚠️ Error de permisos.');
  ...
  if (REQUIRES_PENDING.includes(action) && render.raw_video.status !== 'rendered_pending_review') {
    await safeAnswerCallback(cbq.id, `⚠️ Este video ya fue procesado (estado actual: ${render.raw_video.status}).`);
  ...
  await safeAnswerCallback(cbq.id, 'Acción no reconocida');
...
// $json (a diferencia del nodo Webhook genérico, que lo envuelve en
// .body) -- si tu versión lo envuelve distinto, ajustar esta línea.
const update = $json;
...
try {
  ...
  // "Supabase caído" u otro fallo inesperado -- no se deja crasheado sin
  // explicación; cada update de Telegram es una ejecución independiente
  // de n8n, así que esto no bloquea las siguientes interacciones.

const profile=$json; const sbHeaders={apikey:$env.SUPABASE_SERVICE_ROLE_KEY,Authorization:`Bearer ${$env.SUPABASE_SERVICE_ROLE_KEY}`,'Content-Type':'application/json',Prefer:'return=representation'};
const claimed=await this.helpers.httpRequest({method:'PATCH',url:`${$env.SUPABASE_URL}/rest/v1/style_profiles?id=eq.${profile.id}&status=eq.pending`,headers:sbHeaders,body:{status:'in_progress'},json:true});
if(!claimed.length)return [];
const prompt=await this.helpers.httpRequest({method:'GET',url:`${$env.EDIT_SPEC_API_URL}/prompts/style-profile-system`,json:true});
const analyses=[];
for(const url of profile.source_video_refs){
 const extracted=await this.helpers.httpRequest({method:'POST',url:`${$env.EDIT_SPEC_API_URL}/video-frames`,body:{video_url:url,positions:[0.1,0.3,0.5,0.7,0.9]},json:true});

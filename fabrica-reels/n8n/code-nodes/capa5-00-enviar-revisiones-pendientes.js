 ...
 *
 * Busca raw_videos en rendered_pending_review que todavía no recibieron
 * el mensaje interactivo de revisión (review_prompt_sent_at IS NULL),
 * arma el mensaje con el link firmado y los 7 botones de acción, y lo
 * manda por Telegram vía HTTP directo a la Bot API (no vía nodo nativo --
 * más robusto entre versiones de n8n, mismo patrón que Capa 3/4).
 *
...
for (const rawVideo of pending) {
  ...
  if (!render) continue; // defensivo: no debería pasar si el estado es consistente
  ...
  const replyMarkup = {
    inline_keyboard: [
      ...
      [
        { text: '🔁 Variante automática', callback_data: `varr:${render.id}` },
        { text: '✏️ Pedir cambios', callback_data: `chng:${render.id}` },
      ],
      [
